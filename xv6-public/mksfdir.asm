...
_mksfdir:     file format elf32-i386
...
int main(int argc, char *argv[]){
  ...
  exit();
  5a:	e8 55 02 00 00       	call   2b4 <exit>
...
int main(int argc, char *argv[]){
  ...
  if (argc < 2 ){
    printf(2, "Usage: mksfdir <filenames>\n");
  5f:	c7 44 24 04 ee 06 00 	movl   $0x6ee,0x4(%esp)
  66:	00 
  67:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
  6e:	e8 9d 03 00 00       	call   410 <printf>
  73:	eb e5                	jmp    5a <main+0x5a>
  75:	90                   	nop
  76:	90                   	nop
  77:	90                   	nop
  78:	90                   	nop
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 3bc:	f7 f1                	div    %ecx
 3be:	0f b6 92 2f 07 00 00 	movzbl 0x72f(%edx),%edx
  }while((x /= base) != 0);
 ...
 427:	8d 45 10             	lea    0x10(%ebp),%eax
{
 ...
 4f0:	89 45 e4             	mov    %eax,-0x1c(%ebp)
        if(s == 0)
          s = "(null)";
 4f3:	b8 28 07 00 00       	mov    $0x728,%eax
...
free(void *ap)
{
 ...
 571:	a1 e0 09 00 00       	mov    0x9e0,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
 ...
 5e2:	03 53 fc             	add    -0x4(%ebx),%edx
    ...
    p->s.ptr = bp;
  freep = p;
 5e5:	a3 e0 09 00 00       	mov    %eax,0x9e0
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 609:	8b 5d 08             	mov    0x8(%ebp),%ebx
  if((prevp = freep) == 0){
 60c:	8b 15 e0 09 00 00    	mov    0x9e0,%edx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
morecore(uint nu)
{
 ...
 647:	76 57                	jbe    6a0 <malloc+0xa0>
    ...
    if(p == freep)
 649:	3b 05 e0 09 00 00    	cmp    0x9e0,%eax
...
morecore(uint nu)
{
 ...
 67d:	e8 ee fe ff ff       	call   570 <free>
  return freep;
 682:	8b 15 e0 09 00 00    	mov    0x9e0,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 6ac:	89 58 04             	mov    %ebx,0x4(%eax)
      ...
      freep = prevp;
 6af:	89 15 e0 09 00 00    	mov    %edx,0x9e0
    }
    if(p == freep)
      if((p = morecore(nunits)) == 0)
        return 0;
 ...
 6c4:	eb e9                	jmp    6af <malloc+0xaf>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 6c6:	c7 05 e0 09 00 00 e4 	movl   $0x9e4,0x9e0
 6cd:	09 00 00 
    base.s.size = 0;
 6d0:	b8 e4 09 00 00       	mov    $0x9e4,%eax
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 6d5:	c7 05 e4 09 00 00 e4 	movl   $0x9e4,0x9e4
 6dc:	09 00 00 
    base.s.size = 0;
 6df:	c7 05 e8 09 00 00 00 	movl   $0x0,0x9e8
 6e6:	00 00 00 
